...
def computeDice(autoSeg, groundTruth):
    ...
    DiceArray : floats array
          ...
          No similarity = 0 """
    autoSeg = sitk.ReadImage(autoSeg)
    groundTruth = sitk.ReadImage(groundTruth)
    autoSeg = sitk.GetArrayFromImage(autoSeg)
    groundTruth = sitk.GetArrayFromImage(groundTruth)
    autoSeg = np.squeeze(autoSeg)
    groundTruth = np.squeeze(groundTruth)

    #print(autoSeg.shape, groundTruth.shape)
    #groundTruth = groundTruth[16:-16,16:-16,8:-8]
    #n_classes = int( np.max(groundTruth) + 1)
    n_classes = np.unique(groundTruth)
    labels = np.unique(groundTruth)
    DiceArray = []

    #for c_i in range(1,n_classes):
    #print(n_classes[1:55])
    for c_i in n_classes[1:]:
        idx_Auto = np.where(autoSeg.flatten() == c_i)[0]
        idx_GT   = np.where(groundTruth.flatten() == c_i)[0]
        
        autoArray = np.zeros(autoSeg.size,dtype=np.bool_)
        autoArray[idx_Auto] = 1
        
        gtArray = np.zeros(autoSeg.size,dtype=np.bool_)
        gtArray[idx_GT] = 1
