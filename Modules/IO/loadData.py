...
def sitkLoadImage(imageName):
    image = sitk.ReadImage(imageName)
    imageData = sitk.GetArrayFromImage(image)
    #imageData = nib.load(imageName).get_data()
    '''
    cropx,cropy,cropz = 192,160,144
    x, y, z = imageData.shape
    startx = x//2 - cropx//2
    starty = y//2 - cropy//2
    startz = z//2 - cropz//2
    imageData = imageData[startx:startx+cropx, starty:starty+cropy, startz:startz+cropz]
    '''
    org = image.GetOrigin()
    #print(org)
    return imageData, org
